...
class ContentLossLayer(object):
    ...
    def forward(self, input_layer, content_layer):
        ...
        idxn = input_layer.shape[0]
        idxc = input_layer.shape[1]
        idxh = input_layer.shape[2]
        idxw = input_layer.shape[3]
        loss = np.sum(np.square(input_layer - content_layer)) / (2 * idxn * idxc * idxh * idxw)
        return loss
    ...
    def backward(self, input_layer, content_layer):
        ...
        idxh = input_layer.shape[2]
        idxw = input_layer.shape[3]
        bottom_diff = (input_layer - content_layer) / (idxn * idxc * idxh * idxw)
        return bottom_diff
...
class StyleLossLayer(object):
    ...
    def backward(self, input_layer, style_layer):
        ...
        for idxn in range(input_layer.shape[0]):
            ...
            bottom_diff[idxn, :, :] = np.dot(diff, self.input_layer_reshape[idxn, :, :]) / (
