...
class MNIST_MLP(object):
    def __init__(self, batch_size=75, input_size=784, hidden1=300, hidden2=64, hidden3=16, out_classes=10,
                 lr=0.0075, max_epoch=20, print_iter=100):
    ...
    def build_model(self):  # 建立网络结构
        # TODO：建立三层神经网络结构
        print('Building multi-layer perception model...', self.hidden1, self.hidden2, self.hidden3)
...
def build_mnist_mlp(param_dir='weight.npy'):
    mlp = MNIST_MLP(batch_size=10000)
    mlp.load_data()
    mlp.build_model()
    mlp.init_model()
    # mlp.train()
